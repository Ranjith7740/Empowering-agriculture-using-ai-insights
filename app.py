...
def preprocess_images(pest):
    try:
        ...
        # result = model.predict_classes(img_preprocessed )
        # return result
...
def predict():
    if request.method == 'POST':
        ...
        pred = preprocess_images(pest=file_path)

        if isinstance(pred,str) and pred == 'x':
            return render_template('unaptfile.html')
        
        prediction = np.argmax(model.predict(pred),axis=1)
